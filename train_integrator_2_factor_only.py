...
N_TIMECONSTS_PAIR = 4
N_TIMECONSTS_TRIPLET = 16
N_TIMECONSTS = 3 * N_TIMECONSTS_PAIR + 2 * N_TIMECONSTS_TRIPLET
...
if __name__ == '__main__':
	...
	if not args.train or len(existing_dirs_with_run_num) == 0:
		...
		if args.train and len(existing_dirs_with_run_num) == 0: # if starting a new training run, create options, initial condition, evolutionary strategy
			...
			x0 = np.concatenate([np.zeros(N_RULES - N_THREE_FACTOR_RULES_PER_TYPE), 5e-3 * np.ones(N_TIMECONSTS - N_TIMECONSTS_TRIPLET)])
	...
	if not args.train:
		...
		def to_full_rule(x):
			end_pairwise = 3 * (N_PAIRWISE_RULES_PER_TYPE + N_SUMMED_WEIGHT_RULES_PER_TYPE)
			x_full = np.concatenate([np.zeros(N_RULES), 5e-3 * np.ones(N_TIMECONSTS)])
			x_full[:end_pairwise] = x[:end_pairwise]
		...
		while not es.stop():
			X = es.ask()
			print(X)
			es.tell(X, eval_all([to_full_rule(x) for x in X], eval_tracker=eval_tracker))
