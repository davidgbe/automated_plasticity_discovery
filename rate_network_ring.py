...
def simulate(t : np.ndarray, n_e : int, n_i : int, inp : np.ndarray, plasticity_coefs : np.ndarray, w : np.ndarray, w_plastic : np.ndarray, tau_e=5e-3, tau_i=5e-3, tau_stdp=5e-3, dt=1e-6, g=1, w_u=1, w_scale_factor=1., track_params=False):    
    ...
    r = np.zeros((len_t, 3 * n_e + n_i))
    s = np.zeros((len_t, 3 * n_e + n_i))
    v = np.zeros((len_t, 3 * n_e + n_i))
    r_exp_filtered = np.zeros((len_t, n_e + n_i))

    sign_w = np.where(w >= 0, 1, -1).astype(int)
    inf_w = np.where(sign_w >= 0, 1e-6, -1e-6)

    tau = np.concatenate([tau_e * np.ones(3 * n_e), tau_i * np.ones(n_i)])
...
def simulate_inner_loop(
    ...
    for i in range(0, len(t) - 1):
        ...
        shifted_s_e = s[i, :3 * n_e] - 0.1
        shifted_s_e[shifted_s_e < 0] = 0
        r[i+1, :3 * n_e] = g * np.tanh(shifted_s_e)

        shifted_s_i = s[i, -n_i:] - 0.1
        shifted_s_i[shifted_s_i < 0] = 0
        r[i+1, -n_i:] = g * np.tanh(shifted_s_i)
        
        # calculate exponential filtered of firing rate to use for STDP-like plasticity rules
        r_exp_filtered[i+1, :] = r_exp_filtered[i, :] * (1 - dt / tau_stdp) + r[i, 2 * n_e:] * (dt / tau_stdp)

        r_0_pow = np.ones(n_e + n_i)
        r_1_pow = r[i+1, 2 * n_e:] / 0.1
        r_2_pow = np.square(r[i+1, 2 * n_e:]) / 0.01
        ...
        w_updates_unweighted = np.concatenate((r_cross_products, w_copy[2 * n_e:, 2 * n_e:] / w_scale_factor * r_cross_products))

        if track_params:
            dw_e_e_unsummed = plasticity_coefs[:one_third_n_params].reshape(one_third_n_params, 1, 1) * (w_updates_unweighted[:, :n_e, :n_e] * w_plastic[:n_e, :n_e])
            ...
            dw_e_i_unsummed = plasticity_coefs[one_third_n_params:2*one_third_n_params].reshape(one_third_n_params, 1, 1) * (w_updates_unweighted[:, n_e:, :n_e] * w_plastic[n_e:, :n_e])
            ...
            dw_i_e_unsummed = plasticity_coefs[2 * one_third_n_params:].reshape(one_third_n_params, 1, 1) * (w_updates_unweighted[:, :n_e, n_e:] * w_plastic[:n_e, n_e:])
            ...
            dw_e_e = np.sum(plasticity_coefs[:one_third_n_params].reshape(one_third_n_params, 1, 1) * w_updates_unweighted[:, :n_e, :n_e], axis=0) * w_plastic[2 * n_e:3 * n_e, 2 * n_e:3 * n_e]
            dw_e_i = np.sum(plasticity_coefs[one_third_n_params:2*one_third_n_params].reshape(one_third_n_params, 1, 1) * w_updates_unweighted[:, n_e:, :n_e], axis=0) * w_plastic[-n_i:, 2 * n_e:3 * n_e]
            dw_i_e = np.sum(plasticity_coefs[2 * one_third_n_params:].reshape(one_third_n_params, 1, 1) * w_updates_unweighted[:, :n_e, n_e:], axis=0) * w_plastic[2 * n_e:3 * n_e, -n_i:]

        w_copy[2 * n_e:3 * n_e, 2 * n_e:3 * n_e] += (0.0005 * dw_e_e)
        w_copy[-n_i:, 2 * n_e:3 * n_e] += (0.0005 * dw_e_i)
        w_copy[2 * n_e:3 * n_e, -n_i:] += (0.0005 * dw_i_e)
