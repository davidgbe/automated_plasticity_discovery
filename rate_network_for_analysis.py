...
def simulate(t : np.ndarray, n_e_pool : int, n_e_side : int, n_i : int, inp : np.ndarray, plasticity_coefs : np.ndarray, rule_time_constants : np.ndarray, w : np.ndarray, w_plastic : np.ndarray, tau_e=5e-3, tau_i=5e-3, dt=1e-6, g=1, w_u=1, track_params=False):    
    ...
    w_copy, effects, all_w = simulate_inner_loop(t, n_e_pool, n_e_side, n_i, inp, plasticity_coefs, rule_time_constants, w, w_plastic, dt, g, w_u, track_params, r, s, v, r_exp_filtered, sign_w, inf_w, tau, n_params)

    return r, s, v, w_copy, effects, r_exp_filtered, all_w
...
def simulate_inner_loop(
    ...
    w_copy = np.copy(w)
    all_w = np.empty((len(t), w.shape[0], w.shape[1]))
    ...
    for i in range(0, len(t) - 1):
        ...
        for k, pop_indices in enumerate([[0, 0, 1], [1, 0, 0]]):
            ...
            third_factor = np.dot(w_k, r_exp_filtered_curr_split[p_l][ts_for_pop_start + 3, :]) / 0.5
            ...
            third_factor = np.dot(w_k, r_exp_filtered_curr_split[p_l][ts_for_pop_start + 5, :]) / 0.5
            ...
            third_factor = np.dot(w_k, r_exp_filtered_curr_split[p_l][ts_for_pop_start + 7, :]) / 0.5
            ...
            third_factor = np.dot(w_k, r_exp_filtered_curr_split[p_l][ts_for_pop_start + 1 + tc_offset, :]) / 0.5
            ...
            third_factor = np.dot(w_k, r_exp_filtered_curr_split[p_l][ts_for_pop_start + 3 + tc_offset, :]) / 0.5
            ...
            third_factor = np.dot(w_k, r_exp_filtered_curr_split[p_l][ts_for_pop_start + 5 + tc_offset, :]) / 0.5
            ...
            third_factor = np.dot(w_k, r_exp_filtered_curr_split[p_l][ts_for_pop_start + 7 + tc_offset, :]) / 0.5
        ...
        all_w[i, ...] = np.copy(w_copy)

    return w_copy, effects, all_w
