...
parser.add_argument('--title', metavar='ttl', type=str, default='')
...
parser.add_argument('--batch', metavar='b', type=int, default=1, help='Number of simulations that should be batched per loss function evaluation')
parser.add_argument('--fixed_data', metavar='fd', type=int, default=1, help='')
...
N_INNER_LOOP_RANGE = (1400, 1401) # Number of times to simulate network and plasticity rules per loss function evaluation
DECODER_TRAIN_ITERS = [1199, 1194, 1189, 1184, 1179, 1174]
DECODER_TEST_ITERS = [1399, 1394, 1389, 1384, 1379, 1374]
DW_LAG = 5
...
out_dir = f'sims_out/ila_rules_det_input_test_{TITLE}_BATCH_{BATCH_SIZE}_FIXED_{FIXED_DATA}_L1_PENALTY_{joined_l1}_CHANGEP_{CHANGE_PROB_PER_ITER}_FRACI_{FRAC_INPUTS_FIXED}_SEED_{SEED}_{time_stamp}'
os.mkdir(out_dir)
...
def simulate_single_network(index, x, train, track_params=True):
	...
	fixed_inputs_spks[10:int(input_end), 1:n_e + n_i] = np.random.poisson(lam=INPUT_RATE_PER_CELL * FRAC_INPUTS_FIXED * dt, size=(int(input_end) - 10, n_e - 1 + n_i))
	...
	for i in range(n_inner_loop_iters):
		...
		random_inputs_poisson[10:int(input_end), :n_e + n_i] = np.random.poisson(lam=INPUT_RATE_PER_CELL * (1 - FRAC_INPUTS_FIXED) * dt, size=(int(input_end) - 10, n_e + n_i))
