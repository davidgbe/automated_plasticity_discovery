...
DECODER_TEST_ITERS = [449, 444, 439, 434, 429, 424]
...
out_dir = f'sims_out/stress_test_single_cell_{BATCH_SIZE}_STD_EXPL_{STD_EXPL}_FIXED_{FIXED_DATA}_L1_PENALTY_{joined_l1}_ACT_PEN_{args.asp}_CHANGEP_{CHANGE_PROB_PER_ITER}_FRACI_{FRAC_INPUTS_FIXED}_SEED_{SEED}_{time_stamp}'
os.mkdir(out_dir)
...
def simulate_single_network(index, x, train, track_params=True):
	...
	for i in range(n_inner_loop_iters):
		...
		if i == 400:
			...
			target_cell_idx = np.nonzero(t_ordering == 13)[0][0]
			w_out[target_cell_idx, :n_e] *= 0.1
