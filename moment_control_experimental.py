...
parser.add_argument('--inh_m2_c', metavar='im2c', type=float)

# input params
parser.add_argument('--a_i', type=float)
parser.add_argument('--T_i', type=float)
parser.add_argument('--a_f', type=float)
...
out_dir = f'sims_out/2MC_strong_inh_v4_INDEX_{INDEX}_{args.title}_{time_stamp}'
os.mkdir(out_dir)
	...
	for i in np.arange(BATCH_SIZE):
		...
		for trial_idx in range(rs_for_loss.shape[0]):
			...
			for l_idx in range(r.shape[1]):
				if l_idx < n_e:
					if trial_idx < (DECODER_TRAIN_RANGE[1] - DECODER_TRAIN_RANGE[0]):
...
if __name__ == '__main__':
	...
	x_test[47] = args.inh_m2_c * inh_rule_rate
